...
{
     For W9825G6KH: 4096 rows, typical refresh time = 64ms
     Refresh Count = (Refresh Time * SDRAM Clock Frequency) / Number of rows

     SDRAMClockFreqMHz is in MHz, convert to KHz for calculation
    uint32_t sdram_freq_khz = SDRAMClockFreqMHz * 1000;
    uint32_t refresh_count = (RefreshTimeMs * sdram_freq_khz) / 4096;

     Datasheet suggests subtracting 20 cycles for timing margin
    if (refresh_count > 20) {
        refresh_count -= 20;
    }

     Ensure minimum refresh count
    if (refresh_count < 100) {
        refresh_count = 100;
    }

    printf("Refresh Calculation:\n");
    printf("  Clock Frequency: %lu MHz\n", SDRAMClockFreqMHz);
    printf("  Refresh Time: %lu ms\n", RefreshTimeMs);
    printf("  Number of Rows: 4096\n");
...
W9825G6KH_StatusTypeDef W9825G6KH_SetRefreshRate(uint32_t RefreshRate)
{
    ...
    printf("Setting refresh rate to %lu... ", RefreshRate);

    if (HAL_SDRAM_ProgramRefreshRate(hsdram_ptr, RefreshRate) != HAL_OK) {
        printf("FAILED\n");
        return W9825G6KH_ERROR;
    }

    DeviceConfig.RefreshRate = RefreshRate;
    printf("OK\n");
...
W9825G6KH_StatusTypeDef W9825G6KH_EnterSelfRefresh(void)
{
    ...
    printf("Entering self-refresh mode... ");
...
W9825G6KH_StatusTypeDef W9825G6KH_ExitSelfRefresh(void)
{
    ...
    HAL_Delay(1);  /* Wait for SDRAM to stabilize */
    printf("OK\n");
    return W9825G6KH_OK;
}
...
/**
  * @brief  Gets total SDRAM size in bytes
  * @retval SDRAM size in bytes
  */
uint32_t W9825G6KH_GetSize(void)
{
    return sdram_size_bytes;
}

/**
  * @brief  Checks if address is valid
  * @param  Address: Address to check (offset from SDRAM base)
  * @retval W9825G6KH status
  */
W9825G6KH_StatusTypeDef W9825G6KH_CheckAddress(uint32_t Address)
{
    return W9825G6KH_CheckAddressRange(Address, 1);
}

/**
  * @brief  Get SDRAM status
  * @retval W9825G6KH status
  */
W9825G6KH_StatusTypeDef W9825G6KH_GetStatus(void)
{
    if (hsdram_ptr == NULL) {
        printf("SDRAM Status: NOT INITIALIZED\n");
        return W9825G6KH_ERROR;
    }

    HAL_SDRAM_StateTypeDef state = HAL_SDRAM_GetState(hsdram_ptr);

    switch (state) {
        case HAL_SDRAM_STATE_RESET:
            printf("SDRAM Status: RESET\n");
            break;
        case HAL_SDRAM_STATE_READY:
            printf("SDRAM Status: READY\n");
            break;
        case HAL_SDRAM_STATE_BUSY:
            printf("SDRAM Status: BUSY\n");
            break;
        case HAL_SDRAM_STATE_ERROR:
            printf("SDRAM Status: ERROR\n");
            break;
        default:
            printf("SDRAM Status: UNKNOWN (%d)\n", state);
            break;
    }

    return (state == HAL_SDRAM_STATE_READY) ? W9825G6KH_OK : W9825G6KH_BUSY;
}
...
{
    switch (status) {
        case W9825G6KH_OK: return "OK";
        case W9825G6KH_ERROR: return "ERROR";
        case W9825G6KH_BUSY: return "BUSY";
        case W9825G6KH_TIMEOUT: return "TIMEOUT";
        case W9825G6KH_INVALID_PARAM: return "INVALID_PARAM";
        default: return "UNKNOWN";
    }
}

/**
  * @brief  Dumps current configuration
  * @retval W9825G6KH status
  */
W9825G6KH_StatusTypeDef W9825G6KH_DumpConfig(void)
{
    if (hsdram_ptr == NULL) {
        printf("SDRAM Configuration: NOT INITIALIZED\n");
        return W9825G6KH_ERROR;
    }

    printf("=== SDRAM Configuration ===\n");
    printf("  Base Address: 0x%08lX\n", W9825G6KH_BANK_ADDR);
    printf("  Size: %lu MB (%lu bytes)\n",
           W9825G6KH_SIZE_MB, sdram_size_bytes);
    printf("  Target Bank: %s\n",
           DeviceConfig.TargetBank == FMC_SDRAM_CMD_TARGET_BANK1 ? "BANK1" : "BANK2");
    printf("  Refresh Rate: %lu\n", DeviceConfig.RefreshRate);
    printf("  CAS Latency: %s\n",
           DeviceConfig.CASLatency == W9825G6KH_MR_CAS_LATENCY_2 ? "2" : "3");
    printf("  Burst Length: ");
    switch(DeviceConfig.BurstLength) {
        case W9825G6KH_MR_BURST_LENGTH_1: printf("1\n"); break;
        case W9825G6KH_MR_BURST_LENGTH_2: printf("2\n"); break;
        case W9825G6KH_MR_BURST_LENGTH_4: printf("4\n"); break;
        case W9825G6KH_MR_BURST_LENGTH_8: printf("8\n"); break;
        default: printf("Unknown\n"); break;
    }
    printf("  Burst Type: %s\n",
           DeviceConfig.BurstType == W9825G6KH_MR_BURST_TYPE_SEQUENTIAL ?
           "Sequential" : "Interleaved");
    printf("  Write Burst Mode: %s\n",
           DeviceConfig.WriteBurstMode == W9825G6KH_MR_WRITE_BURST_MODE_SINGLE ?
           "Single Location" : "Programmed Burst");
...
/**
  * @brief  Debug function to read mode register indirectly
  * @retval W9825G6KH status
  */
W9825G6KH_StatusTypeDef W9825G6KH_DebugReadModeRegister(void)
{
    ...
    uint32_t *pSdram = (uint32_t *)W9825G6KH_BANK_ADDR;
    uint32_t dummy_read = pSdram[0];

    printf("[DEBUG] Dummy read from 0x%08lX = 0x%08lX\n",
           (uint32_t)pSdram, dummy_read);
...
/**
  ...
  * @param  test_size: Size in bytes to test
  * @retval W9825G6KH status
  */
W9825G6KH_StatusTypeDef W9825G6KH_RunDiagnostics(uint32_t test_size)
{
    ...
    if (hsdram_ptr == NULL) {
        printf("ERROR: SDRAM not initialized\n");
        return W9825G6KH_ERROR;
    }

    /* Dump configuration */
    W9825G6KH_DumpConfig();

    /* Get status */
    W9825G6KH_GetStatus();

    /* Run memory test */
    printf("\nRunning memory test (%lu bytes)...\n", test_size);
    W9825G6KH_StatusTypeDef status = W9825G6KH_MemoryTest(0, test_size);

    printf("Memory test: %s\n", W9825G6KH_StatusToString(status));
    ...
    uint32_t test_data[4] = {0x12345678, 0x87654321, 0xAAAAAAAA, 0x55555555};
    uint32_t read_data[4];

    for (int i = 0; i < 4; i++) {
        status = W9825G6KH_WriteBuffer32(&test_data[i], i * 16, 1);
        if (status != W9825G6KH_OK) {
            printf("  Write pattern %d failed: %s\n",
                   i, W9825G6KH_StatusToString(status));
            return status;
        }

        status = W9825G6KH_ReadBuffer32(&read_data[i], i * 16, 1);
        if (status != W9825G6KH_OK) {
            printf("  Read pattern %d failed: %s\n",
                   i, W9825G6KH_StatusToString(status));
            return status;
        }

        if (read_data[i] != test_data[i]) {
            printf("  Pattern %d mismatch: wrote 0x%08lX, read 0x%08lX\n",
                   i, test_data[i], read_data[i]);
            return W9825G6KH_ERROR;
        }
    }

    printf("All access patterns: PASS\n");

    /* Test refresh functionality */
    printf("\nTesting refresh functionality...\n");
    uint32_t test_addr = 0x00100000;
    uint32_t test_value = 0xDEADBEEF;
    uint32_t read_value;

    status = W9825G6KH_WriteBuffer32(&test_value, test_addr, 1);
    if (status != W9825G6KH_OK) {
        printf("  Refresh test write failed\n");
        return status;
    }

    /* Wait for some refresh cycles */
    HAL_Delay(1000);

    status = W9825G6KH_ReadBuffer32(&read_value, test_addr, 1);
    if (status != W9825G6KH_OK) {
        printf("  Refresh test read failed\n");
        return status;
    }

    if (read_value == test_value) {
        printf("  Refresh test: PASS (data retained)\n");
    } else {
        printf("  Refresh test: FAIL (data corrupted: 0x%08lX)\n", read_value);
        return W9825G6KH_ERROR;
    }
...
/*W9825G6KH_StatusTypeDef W9825G6KH_Init(SDRAM_HandleTypeDef *hsdram_param, W9825G6KH_InitTypeDef *Config)
{
    ...
    printf("4. Loading Mode Register...\n");
        ...
        mode_register = Config->BurstLength |
                        Config->BurstType |
                        cube_mx_cas |  // Use CubeMX's setting!
                        Config->OperatingMode |
                        Config->WriteBurstMode;
    ...
    printf("5. Setting Refresh Rate to %lu...\n", Config->RefreshRate);
    if (HAL_SDRAM_ProgramRefreshRate(hsdram_param, Config->RefreshRate) != HAL_OK) {
        printf("  ERROR: Set Refresh Rate failed\n");
        return W9825G6KH_ERROR;
    }
